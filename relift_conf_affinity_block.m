...
%RELIFT_CONF_AFFINITY_BLOCK Compute confusion affinity for a candidate block.

labels = block.pair_leaf_labels(:)';
num_labels = numel(labels);
A_conf = zeros(num_labels, num_labels);

sample_idx = ismember(dev_data(:, end), ...
    relift_leaf_descendants(state.tree, block.parent));
if ~any(sample_idx)
    return;
end

oracle_pred = relift_predict_from_node( ...
    dev_data(sample_idx, 1:end-1), state, block.parent);
...
for i = 1:numel(local_true)
    true_idx = find(labels == local_true(i), 1);
    pred_idx = find(labels == oracle_pred(i), 1);
    if ~isempty(true_idx) && ~isempty(pred_idx) && true_idx ~= pred_idx
        A_conf(true_idx, pred_idx) = A_conf(true_idx, pred_idx) + weights(i);
    end
end
